...
export async function POST(req) {
  ...
  if (!data) return NextResponse.json({ error: "Error" }, { status: 200 });
  ...
  for (const [k, v] of Object.entries(data.checkOutOptions)) {
    const prices = await stripe.prices.search({
      query: `metadata['item_title']:'${data.title}' AND metadata['option_title']:'${v.title}'`,
    });
    if (prices.data.length == 1) {
      items.push({ price: prices.data.id, quantity: data.count });
    } else {
      // error
    }
  }
